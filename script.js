...
function showMessage(msg, isError=false) {
  ...
  resultDiv.style.color = isError ? 'crimson' : 'inherit';
}
...
function calcMetric(w, hCm) {
  const h = hCm / 100;
  return w / (h * h);
}
function calcImperial(lb, ft, inch) {
...
copyBtn.addEventListener('click', async () => {
  ...
  try {
    await navigator.clipboard.writeText(JSON.parse(last).bmi + ' BMI');
...
shareBtn.addEventListener('click', async () => {
  ...
  if (navigator.share) {
    try { await navigator.share({title:'BMI Result', text}); showMessage('Shared!'); }
    catch(e){ showMessage('Share cancelled', true); }
  } else {
